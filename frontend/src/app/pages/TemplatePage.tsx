      ...
      {TEMPLATE_STEPS.map((item, index) => {
        ...
        return (
          <div key={item.key} className="flex items-center gap-2">
            ...
            <span className={`hidden text-xs font-medium md:block ${active ? "text-white" : "text-gray-500"}`}>
              {stepLabels[item.key]}
            </span>
            {index < TEMPLATE_STEPS.length - 1 && <span className="hidden h-px w-5 bg-[#1F1F1F] md:block" />}
          </div>
        );
      })}
    </div>
      ...
      return (
        <div>
          <div className="mb-8 text-center">
            <h1 className="text-3xl font-bold text-white sm:text-4xl">{ui.template.chooseTitle}</h1>
            <p className="mt-2 text-gray-400">{ui.template.chooseSubtitle}</p>
          </div>
          ...
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full rounded-lg border border-[#2A2A2A] bg-[#141414] py-3 pl-10 pr-4 text-sm text-white placeholder:text-gray-500 focus:border-[#7C3AED] focus:outline-none"
            />
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border p-4 text-left transition-all ${
                    active ? "border-[#7C3AED] bg-[#7C3AED]/10" : "border-[#2A2A2A] bg-[#141414] hover:border-[#7C3AED]/50"
                  }`}
                >
                  <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-lg ${
                    active ? "bg-[#7C3AED]/20 text-[#7C3AED]" : "bg-[#1A1A1A] text-gray-400"
                  }`}>
                    <Icon size={20} />
                  </div>
                  <h3 className={`mb-1 text-sm font-semibold ${active ? "text-[#7C3AED]" : "text-white"}`}>{group.name}</h3>
                  <p className="text-xs text-gray-500">{group.items.length} {ui.template.fallbackDescription}</p>
                </button>
              );
            })}
          </div>
          ...
          <div className="rounded-xl border border-[#2A2A2A] bg-[#141414] p-6">
            <div className="mb-4">
              <h2 className="text-lg font-semibold text-[#7C3AED]">{activeTemplateGroup.name}</h2>
              <p className="text-sm text-gray-400">{activeTemplateGroup.desc}</p>
            </div>
                ...
                <button
                  ...
                  className={`flex items-start gap-4 rounded-xl border p-4 text-left transition-all ${
                    selectedTemplate === card.name
                      ...
                      : "border-[#2A2A2A] bg-[#1A1A1A] hover:border-[#7C3AED]/30"
                  }`}
                >
                  <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
                    selectedTemplate === card.name ? "bg-[#7C3AED]/20 text-[#7C3AED]" : "bg-[#0A0A0A] text-gray-400"
                  }`}>
                    <FileText size={17} />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block text-sm font-medium text-white">{card.name}</span>
                    <span className="mt-1 block text-xs text-gray-500">{card.description || `${card.category}`}</span>
                  </span>
                  <ArrowRight size={15} className="mt-1 shrink-0 text-[#7C3AED]" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mt-6 flex justify-end">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-white px-7 py-3 text-sm font-semibold text-black transition-all hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50"
            >
      ...
      <AnimatePresence>
        {showConfirm && (
          ...
          >
            ...
            >
              <h2 className="mb-8 text-2xl font-bold text-white">{ui.confirm.title}</h2>
              <div className="flex gap-3">
                ...
                <button onClick={() => setShowConfirm(false)} className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-[#2A2A2A] px-5 py-3 text-sm font-semibold text-gray-300 transition-all hover:bg-[#1A1A1A]">
...
function CoffeeButton({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="rounded-lg bg-white px-8 py-3 text-sm font-semibold text-black transition-all hover:bg-gray-200">
      {children}
    </button>
  );
}
...
function StepActions({ ui, onBack, onContinue }: { ui: UiContent; onBack: () => void; onContinue: () => void }) {
  return (
    <div className="mt-8 flex items-center justify-between gap-4">
      <button type="button" onClick={onBack} className="flex items-center gap-2 rounded-lg border border-[#2A2A2A] px-6 py-3 text-sm font-medium text-gray-300 transition-all hover:bg-[#1A1A1A]">
...
function TemplateField({ variable, value, onChange }: {
  ...
  const baseClass = "w-full rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] px-4 py-3 text-sm text-white outline-none transition-all placeholder:text-gray-500 focus:border-[#7C3AED]";
  const label = variable.label || variable.key;

  return (
    <label className="block">
      <span className="mb-2 flex items-center gap-2 text-xs font-medium text-gray-400">
        <span>{label}</span>
        {variable.required && <span className="text-[#7C3AED]">*</span>}
      </span>
      {variable.type === "textarea" ? (
        ...
        <button type="button" onClick={() => onChange(value === "true" ? "false" : "true")}
          className={`flex h-12 w-full items-center justify-between rounded-lg border px-4 text-sm font-medium ${
            value === "true" ? "border-[#7C3AED] bg-[#7C3AED]/10 text-[#7C3AED]" : "border-[#2A2A2A] bg-[#0A0A0A] text-gray-400"
          }`}
        >
          <span>{label}</span>
          <span className={`h-5 w-9 rounded-full p-0.5 transition-colors ${value === "true" ? "bg-[#7C3AED]" : "bg-[#2A2A2A]"}`}>
...
function TemplateDetails({ template, values, ui, onValueChange, onBack, onContinue }: {
  ...
  return (
    <motion.div key="details" className="flex flex-col">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-white">{template?.name || "Selected template"}</h1>
        <p className="mt-2 text-gray-400">{ui.template.editor.helper}</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        <div className="rounded-xl border border-[#2A2A2A] bg-[#141414]">
          <div className="flex items-center justify-between border-b border-[#2A2A2A] px-5 py-4">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-gray-500">{ui.template.editor.preview}</p>
              <h2 className="mt-1 text-sm font-semibold text-white">{template?.category || "Contract"}</h2>
            </div>
            <span className="rounded-md bg-[#7C3AED]/10 px-3 py-1 text-xs font-medium text-[#7C3AED]">AI</span>
          </div>
        ...
        <div className="rounded-xl border border-[#2A2A2A] bg-[#141414]">
          <div className="border-b border-[#2A2A2A] px-5 py-4">
            <p className="text-xs font-medium uppercase tracking-wider text-gray-500">{ui.template.editor.fields}</p>
            <h2 className="mt-1 text-lg font-bold text-white">{template?.variables.length || 0} {ui.template.editor.fieldUnit}</h2>
          </div>
          <div className="px-5 py-5">
            {groups.length === 0 ? (
              <p className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] p-4 text-sm text-gray-400">{ui.template.editor.emptyFields}</p>
            ) : (
              <div className="space-y-6">
                {groups.map(group => (
                  <div key={group.title}>
                    <h3 className="mb-3 text-sm font-semibold text-[#7C3AED]">{group.title}</h3>
...
function TemplateVerification({ template, values, ui, onBack, onContinue }: {
  template?: TemplateSummary; values: Record<string, string>; ui: UiContent; onBack: () => void; onContinue: () => void;
}) {
  const preview = renderTemplateContent(template, values, ui);
  ...
  return (
    <motion.div key="verification" className="flex flex-col">
      <div className="flex flex-1 items-center justify-center py-12">
        <div className="w-full max-w-3xl rounded-xl border border-[#2A2A2A] bg-[#141414] p-8">
          <p className="mb-2 text-xs font-medium uppercase tracking-wider text-[#7C3AED]">{ui.template.verification.eyebrow}</p>
          <h1 className="mb-6 text-3xl font-bold text-white">{template?.name || ui.template.verification.title}</h1>
          <div className="whitespace-pre-wrap rounded-lg border border-[#2A2A2A] bg-[#0A0A0A] p-6 text-sm leading-7 text-gray-400">
            {preview}
          </div>
        </div>
      </div>
...
function TemplatePayment({ ui, onBack, onContinue }: { ui: UiContent; onBack: () => void; onContinue: () => void }) {
  ...
  return (
    <motion.div key="payment" className="flex flex-col">
      <div className="flex flex-1 items-center justify-center py-12">
        <div className="w-full max-w-md rounded-xl border border-[#2A2A2A] bg-[#141414] p-8 text-center">
          <p className="mb-2 text-xs font-medium uppercase tracking-wider text-[#7C3AED]">QPay</p>
          <h1 className="mb-2 text-2xl font-bold text-white">Төлбөр төлөх</h1>
          <p className="mb-6 text-sm text-gray-400">{amount.toLocaleString("mn-MN")} MNT</p>
          <div className="mx-auto mb-6 flex h-56 w-56 items-center justify-center rounded-xl border border-[#2A2A2A] bg-[#0A0A0A] p-4">
            {isLoading ? (
              <LoaderCircle className="h-14 w-14 animate-spin text-gray-400" strokeWidth={1.5} />
            ) : qrImage ? (
              <img src={qrImage} alt="QPay QR" className="h-full w-full object-contain" />
            ) : (
              <QrCode size={120} strokeWidth={1.25} className="text-gray-500" />
            )}
          </div>
          {invoice?.urls?.length ? (
            <div className="mb-5 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {invoice.urls.filter(item => item.link).slice(0, 6).map(item => (
                <a key={`${item.name}-${item.link}`} href={item.link}
                  className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-3 py-2 text-xs font-medium text-gray-300 transition-colors hover:bg-[#0A0A0A]">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={createInvoice} disabled={isLoading}
              className="flex-1 rounded-lg border border-[#2A2A2A] px-6 py-3 text-sm font-medium text-gray-300 transition-all hover:bg-[#1A1A1A] disabled:opacity-60">
              Дахин үүсгэх
            </button>
...
function TemplateResult({ template, values, ui, onBack, onSave, onExport, onFinish }: {
  ...
  return (
    <motion.div key="template-result" className="grid gap-6 pb-6 lg:grid-cols-[1fr_320px]">
      <div className="rounded-xl border border-[#2A2A2A] bg-[#141414] p-6">
        <p className="mb-2 text-xs font-medium uppercase tracking-wider text-[#7C3AED]">{template?.category || "Contract"}</p>
        <h2 className="mb-6 text-2xl font-bold text-white">{template?.name || ui.template.result.previewTitle}</h2>
        <article className="whitespace-pre-wrap rounded-lg bg-white px-8 py-8 text-sm leading-7 text-gray-900">
          {preview}
        </article>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {[
          ...
          <div key={title} className="rounded-xl border border-[#2A2A2A] bg-[#141414] p-5">
            <p className="mb-2 text-xs font-medium uppercase tracking-wider text-[#7C3AED]">{title}</p>
          ...
          <div className="flex flex-wrap gap-2">
            {["PPT", "Word", "Docs"].map(label => (
              <button key={label} type="button" onClick={onExport}
                className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-4 py-2 text-xs font-medium text-gray-300 transition-all hover:border-[#7C3AED]/50">
                {label}
              </button>
            ))}
            <button type="button" onClick={onSave} className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-4 py-2 text-[#7C3AED] transition-all hover:border-[#7C3AED]/50">
              <Archive size={16} />
            </button>
            <button type="button" onClick={onFinish} className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] px-4 py-2 text-red-400 transition-all hover:border-red-500/50">
              <Trash2 size={16} />
            </button>
          </div>
          <div className="flex items-center justify-between gap-4">
            <button type="button" onClick={onBack} className="rounded-lg border border-[#2A2A2A] px-6 py-3 text-sm font-medium text-gray-300 transition-all hover:bg-[#1A1A1A]">
              {ui.actions.back}
            </button>
