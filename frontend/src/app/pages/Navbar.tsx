        ...
        >
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="rounded bg-accent/10 px-2 py-0.5 text-[10px] font-semibold text-accent">
              {detailItem.type === "document" ? "DOCUMENT" : "CONTRACT"}
            </span>
          </div>

          <h3 className="mb-4 text-lg font-bold text-foreground">{detailItem.data.title || "Untitled"}</h3>
            ...
            <div className="mb-4">
              <div className="mb-1 flex items-center justify-between">
                ...
                {detailItem.type === "document" && !editMode && (
                  <button type="button" onClick={handleStartEdit} className="text-[10px] font-medium text-accent hover:underline">Edit</button>
                )}
              </div>
              <textarea
                readOnly={!editMode}
              ...
              {editMode && (
                <div className="mt-2 flex gap-2">
                  <button type="button" onClick={handleSaveContent} disabled={savingEdit} className="flex items-center gap-1 rounded-lg bg-accent px-3 py-1.5 text-[10px] font-medium text-white transition-all hover:opacity-90 disabled:opacity-50">
                    <Save className="h-3 w-3" /> {savingEdit ? "Saving..." : "Save Content"}
                  </button>
                  <button type="button" onClick={handleCancelEdit} className="rounded-lg border border-border px-3 py-1.5 text-[10px] font-medium text-muted-foreground transition-all hover:bg-muted">Cancel</button>
                </div>
              )}
            </div>
          )}
            ...
            <>
              <div className="mb-3 rounded-lg border border-border bg-muted/50 p-3">
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-bold text-foreground">Risk Analysis</span>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={handleReanalyze} disabled={reanalyzing} className="flex items-center gap-1 text-[10px] font-medium text-accent transition-all hover:underline disabled:opacity-50">
                      <RefreshCw className={`h-3 w-3 ${reanalyzing ? "animate-spin" : ""}`} /> {reanalyzing ? "Analyzing..." : "Re-analyze"}
                    </button>
                    <button type="button" onClick={handleStartEditAnalysis} className="text-[10px] font-medium text-accent hover:underline">Edit</button>
                    <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${
                      ...
                      Score: {detailItem.data.riskAnalysis.riskScore}/100
                    </span>
                  </div>
                </div>
                ...
                {detailItem.data.riskAnalysis.risks?.length > 0 && (
                  <div className="mb-2">
                    <p className="mb-1 text-[10px] font-semibold text-red-400">Risks ({detailItem.data.riskAnalysis.risks.length})</p>
                ...
                {detailItem.data.riskAnalysis.missingClauses?.length > 0 && (
                  <div>
                    <p className="mb-1 text-[10px] font-semibold text-amber-400">Missing Clauses ({detailItem.data.riskAnalysis.missingClauses.length})</p>
                    <ul className="space-y-1">
          ...
          {detailItem.type === "document" && editingAnalysis && (
            <div className="mb-3 rounded-lg border border-border bg-muted/50 p-3">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-xs font-bold text-foreground">Edit Analysis</span>
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={handleCancelEditAnalysis} className="rounded-lg border border-border px-3 py-1.5 text-[10px] font-medium text-muted-foreground transition-all hover:bg-muted">Cancel</button>
                </div>
              </div>
              <div className="space-y-2">
                <div>
          ...
          {detailItem.type === "contract" && detailItem.data.contractType && (
            <div className="mb-3 rounded-lg border border-border bg-muted/50 p-3">
              <p className="mb-1 text-[10px] font-semibold text-muted-foreground">Contract Type</p>
              ...
              {detailItem.data.parties && (
                <div className="mt-2">
                  <p className="mb-1 text-[10px] font-semibold text-muted-foreground">Parties</p>
