...
import { getLegalContext, LegalContext } from "../services/lawService";
...
const emptyLegalContext: LegalContext = {
  relevantLaws: [],
  relevantArticles: [],
  relevantTemplates: [],
};
...
async function loadTemplateFile() {
  ...
  return JSON.parse(raw) as TemplateSource[];
}

export async function listPublicTemplates(_req: Request, res: Response) {
  try {
    const templates = await loadTemplateFile();
    const categories = [...new Set(templates.map((template) => template.category))].map(
      (category) => ({
        name: category,
        items: templates
...
export async function uploadPublicAnalysis(req: Request, res: Response) {
  const file = req.file;
