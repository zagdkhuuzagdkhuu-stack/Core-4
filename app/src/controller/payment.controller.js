...
function createDemoInvoice(amount, description) {
    return {
...
export async function createPublicQPayInvoice(req, res) {
    try {
        const amount = parseAmount(req.body.amount);
        if (!amount) {
            return res.status(400).json({ message: "A valid payment amount is required." });
        ...
        if (canUseDemoQPay()) {
            ...
            const invoice = createDemoInvoice(amount, String(req.body.description || "Draftly document payment").slice(0, 255));
            return res.status(201).json({
                invoice,
                ...invoice,
            });
...
export async function checkQPayPayment(req, res) {
    try {
...
export async function activatePaidAccess(req, res) {
    try {
        const userId = req.userId;
