...
- estimatedCost: estimated contract value in MNT (number or null)
- inconsistentWording: array of inconsistent or contradictory terms found`;
export async function analyzeWithCrewAI(text) {
    ...
    const clauses = (clauseData.clauses || clauseData || []).map((c, i) => ({
...
export async function analyzeWithSingleCall(text) {
    const trimmed = text.slice(0, 50000);
...
export async function analyzeContract(text, mode = "crew") {
